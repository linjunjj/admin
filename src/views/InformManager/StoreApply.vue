<template xmlns:v-popover="http://www.w3.org/1999/xhtml">
  <div class="app-container calendar-list-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="拼命加载中" :height="tableHeight" border stripe fit
      ...
      <el-table-column align="center" width="120px" label="名字" prop="username" sortable>
      </el-table-column>
      <el-table-column align="center"  label="银行账户" prop="bankacount" sortable>
      </el-table-column>
      <el-table-column align="center" width="160px" label="营业执照">
        <template scope="scope">
          <img :src="scope.row.businessimages" style="width: 64px;height: 64px;padding-top: 5px;"/>
        </template>
      </el-table-column>

      <el-table-column align="center" width="160px" label="法人身份证正面">
        <template scope="scope">
          <img :src="scope.row.identityimage" style="width: 64px;height: 64px;padding-top: 5px;"/>
        </template>
      </el-table-column>
      <el-table-column align="center" width="160px" label="法人身份证反面">
        <template scope="scope">
          <img :src="scope.row.identityimages" style="width: 64px;height: 64px;padding-top: 5px;"/>
        </template>
      </el-table-column>
      <el-table-column align="center" width="150px" label="手机号码" prop="tel" sortable>
      </el-table-column>
      <el-table-column align="center" width="180px" label="申请时间" prop="appletime" sortable>
      </el-table-column>

      <el-table-column align="center" label="状态" width="100">
        <template scope="scope">
          <el-tag :type="scope.row.stutas | statusFilter">{{scope.row.stutas | statusFilterTip }}</el-tag>
        </template>
      </el-table-column>
    ...
    methods: {
      ...
      getList() {
        ...
        setTimeout((items, total) => {
          ...
          this.$store.dispatch('GetStoreApplyList',info).then((res)=>{
      ...
      getStatusList(status) {
        ...
        setTimeout((items, total) => {
          ...
          info.page=this.listQuery.page;
          info.pagesize=this.listQuery.limit;
          this.$store.dispatch('GetStatusStoreApply',info).then((res)=>{
            this.total=res.total;
            this.list=res.list;
          })
          this.listLoading = false;
